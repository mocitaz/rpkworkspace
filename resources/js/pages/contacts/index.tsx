...
export default function ContactsIndex({
    ...
    const [createAvatar, setCreateAvatar] = useState<string>('/images/avatars/avatar-1.svg');
    const [editAvatar, setEditAvatar] = useState<string>('');

    // Sync selectedContact and editAvatar when contacts data or editing contact changes
    useEffect(() => {
        if (selectedContact) {
            const found = contacts.data.find((c) => c.id === selectedContact.id);
            if (found) {
    ...
    useEffect(() => {
        if (editingContact) {
            setEditAvatar(editingContact.avatar_url || getContactAvatarUrl(editingContact));
        }
    }, [editingContact]);
    ...
    const toggleStar = (id: string, e?: React.MouseEvent) => {
        ...
        setStarredIds((prev) => {
            ...
            try {
                localStorage.setItem('rpk_starred_contacts', JSON.stringify(next));
            } catch {
                // ignore
            }
            return next;
    ...
    useEffect(() => {
        if (contacts.data.length > 0) {
            if (!selectedContact || !contacts.data.some((c) => c.id === selectedContact.id)) {
                setSelectedContact(contacts.data[0]);
            } else {
                const refreshed = contacts.data.find((c) => c.id === selectedContact.id);
                if (refreshed) {
    ...
    const handleFilterSubmit = (e: React.FormEvent) => {
        ...
        router.get(contactRoutes.index.url(), Object.fromEntries(queryParams.entries()), {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleResetFilters = () => {
        ...
        router.get(contactRoutes.index.url(), {}, { preserveState: true, preserveScroll: true });
    };
    ...
    const isCurrentStarred = selectedContact ? starredIds.includes(selectedContact.id) : false;
    ...
    return (
        <>
                ...
                <main className="mx-auto max-w-7xl space-y-5 px-4 py-5 sm:px-6 lg:px-8">
                        ...
                        {can.create && (
                            <div className="flex shrink-0 items-center gap-2">
                                <Button
                                    ...
                                    className="h-8.5 rounded-lg bg-slate-900 px-3.5 text-xs font-semibold text-white shadow-2xs hover:bg-slate-800 active:scale-95 dark:bg-white dark:text-slate-900 dark:hover:bg-slate-100"
                                >
                    ...
                    <div className="space-y-2 rounded-xl border border-slate-200/60 bg-slate-50/50 p-2.5 dark:border-white/[0.04] dark:bg-[#121418]">
                        <div className="flex flex-wrap items-center gap-2">
                            {/* Search form */}
                            <form onSubmit={handleFilterSubmit} className="relative flex-1 min-w-[220px]">
                                <Search className="pointer-events-none absolute top-1/2 left-2.5 size-3.5 -translate-y-1/2 text-slate-400" />
                                <Input
                                    value={searchQuery}
                                    onChange={(e) => setSearchQuery(e.target.value)}
                                    placeholder="Cari nama personil, jabatan, email, no HP/telepon..."
                                    className="h-8.5 w-full rounded-lg border-slate-200 bg-white pl-8 pr-8 text-xs text-slate-900 placeholder:text-slate-400 focus:border-blue-500 dark:border-white/10 dark:bg-zinc-800 dark:text-white"
                                />
                                {searchQuery && (
                                    <button
                                        type="button"
                            ...
                            {(filters.search || filters.client_id) && (
                                ...
                                >
                                    <RotateCcw className="size-3.5 text-slate-400 mr-1" />
                                    Reset
                                </Button>
                            )}
                    ...
                    {contacts.data.length === 0 ? (
                                ...
                                action={
                                    <div className="flex flex-wrap items-center justify-center gap-2">
                                        ...
                                        {(filters.search || filters.client_id) && (
                                            ...
                                            >
                                                <Link href={contactRoutes.index.url()}>Reset Filter</Link>
                                            </Button>
                                        )}
                                    </div>
                                }
                        ...
                        <div className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-2xs dark:border-white/10 dark:bg-[#14161b]">
                            <div className="flex flex-col lg:flex-row min-h-[620px] lg:h-[calc(100vh-270px)]">
                                
                                {/* LEFT MASTER LIST PANE */}
                                <div
                                    className={`w-full lg:w-[340px] xl:w-[380px] shrink-0 border-r border-slate-200/70 dark:border-white/[0.08] flex flex-col bg-slate-50/40 dark:bg-[#111317] ${
                                    ...
                                    <div className="border-b border-slate-200/70 p-3 bg-white dark:border-white/[0.08] dark:bg-[#14161b] space-y-2">
                                        ...
                                        <div className="flex items-center gap-1">
                                            ...
                                            >
                                                ...
                                                Favorit ({contacts.data.filter((c) => starredIds.includes(c.id)).length})
                                            </button>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex-1 overflow-y-auto divide-y divide-slate-100 dark:divide-white/[0.04]">
                                        {displayedContacts.length === 0 ? (
                                            ...
                                            displayedContacts.map((contact) => {
                                                ...
                                                const isStarred = starredIds.includes(contact.id);
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex items-center gap-3 min-w-0 flex-1">
                                                            <Avatar className="size-10.5 shrink-0 overflow-hidden rounded-full shadow-xs bg-slate-100 dark:bg-zinc-800">
                                                                <AvatarImage
                                                                    src={getContactAvatarUrl(contact)}
                                                                    alt={contact.full_name}
                                                                    className="size-full object-cover"
                                                                />
                                                                <AvatarFallback className="text-xs font-bold bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300">
                                                            ...
                                                            <button
                                                                ...
                                                                className={`flex size-7 items-center justify-center rounded-lg transition-all ${
                                                                    isStarred
                                                                        ...
                                                                        : 'text-slate-300 opacity-60 hover:opacity-100 hover:text-amber-500 dark:text-zinc-600'
                                                                }`}
                                ...
                                <div
                                    className={`flex-1 flex flex-col bg-white dark:bg-[#14161b] overflow-y-auto ${
                                        ...
                                        <div className="flex flex-col h-full">
                                            ...
                                            <div className="flex items-center justify-between border-b border-slate-100 px-6 py-3.5 dark:border-white/[0.06]">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <button
                                                        ...
                                                        className="lg:hidden flex size-8 items-center justify-center rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 dark:border-white/10 dark:text-zinc-300"
                                                    >
                                                ...
                                                <div className="flex items-center gap-0.5">
                                                    ...
                                                    <button
                                                        ...
                                                        title={isCurrentStarred ? 'Hapus Bintang' : 'Tandai Favorit'}
                                                    >
                                                        <Star
                                                            className={`size-4 ${
                                                                isCurrentStarred ? 'fill-amber-400 text-amber-400' : ''
                                                            }`}
                                                        />
                                                    </button>
                                                    ...
                                                    {can.update && (
                                                        <button
                                                            ...
                                                            className="flex size-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-900 dark:hover:bg-zinc-800 dark:hover:text-white transition-all"
                                            ...
                                            <div className="flex-1 px-6 py-5 space-y-6">
                                                ...
                                                <div className="flex items-center gap-4">
                                                    <Avatar className="size-16 sm:size-18 shrink-0 overflow-hidden rounded-full border-2 border-white shadow-xs bg-slate-100 dark:border-zinc-800 dark:bg-zinc-800">
                                                        <AvatarImage
                                                            src={getContactAvatarUrl(selectedContact)}
                                                            alt={selectedContact.full_name}
                                                            className="size-full object-cover"
                                                        />
                                                        <AvatarFallback className="text-lg font-bold bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300">
                                                    ...
                                                    <div className="space-y-0.5 min-w-0">
                                                        ...
                                                        <p className="text-xs font-medium text-slate-600 dark:text-zinc-300">
                                                            {selectedContact.job_title || 'Perwakilan Stakeholder'}
                                                        </p>

                                                        {selectedContact.client ? (
                                                            <Link
                                                                href={clientRoutes.show.url(selectedContact.client.id)}
                                                                className="inline-flex items-center gap-1 text-xs font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 hover:underline pt-0.5"
                                                            >
                                                            ...
                                                            <p className="text-xs text-slate-400 dark:text-zinc-500 pt-0.5">
                                                                {selectedContact.organization_name || 'Independen / Perseorangan'}
                                                            </p>
                                                        )}
                                                    </div>
                                                </div>
                                                ...
                                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 pt-4 border-t border-slate-100 dark:border-white/[0.06]">
                                                    {/* Phone Number */}
                                                        ...
                                                        <div>
                                                            {selectedContact.mobile || selectedContact.phone ? (
                                                                <a
                                                                    ...
                                                                    className="font-mono text-xs sm:text-sm font-semibold text-slate-900 hover:text-blue-600 dark:text-white dark:hover:text-blue-400"
                                                                >
                                                                    {selectedContact.mobile || selectedContact.phone}
                                                                </a>
                                                            ) : (
                                                        ...
                                                        <div>
                                                            {selectedContact.email ? (
                                                                <a
                                                                    ...
                                                                    className="font-mono text-xs sm:text-sm font-semibold text-slate-900 hover:text-blue-600 dark:text-white dark:hover:text-blue-400 break-all"
                                                                >
                                                                    {selectedContact.email}
                                                                </a>
                                                            ) : (
                                        ...
                                        {contacts.data.map((contact) => (
                                            ...
                                            >
                                                <td className="py-2.5 pr-3 pl-4">
                                                    ...
                                                    >
                                                        <Avatar className="size-8 shrink-0 overflow-hidden rounded-full shadow-xs bg-slate-100 dark:bg-zinc-800">
                                                            <AvatarImage
                                                                src={getContactAvatarUrl(contact)}
                                                                alt={contact.full_name}
                                                                className="size-full object-cover"
                                                            />
                                                            <AvatarFallback className="text-[10px] font-bold bg-blue-50 text-blue-700 dark:bg-blue-950 dark:text-blue-300">
                                                                {getInitials(contact.full_name)}
                                                            </AvatarFallback>
                                                        </Avatar>
                                                ...
                                                <td className="px-3 py-2.5 whitespace-nowrap">
                                                    {contact.client ? (
                                                        <Link
                                                            href={clientRoutes.show.url(contact.client.id)}
                                                        ...
                                                        <span className="text-[10px] text-slate-400">Independen</span>
                                                    )}
                                                </td>
            ...
            <Dialog open={openCreate} onOpenChange={setOpenCreate}>
                <DialogContent className="max-h-[88vh] overflow-y-auto rounded-2xl border border-slate-200/80 bg-white p-5 sm:p-6 shadow-xl sm:max-w-lg dark:border-white/10 dark:bg-[#14161b]">
                        ...
                        <div className="flex items-center gap-2.5">
                            ...
                            <div>
                                ...
                                <DialogDescription className="text-xs text-slate-500 dark:text-zinc-400">
                                    Daftarkan representasi klien, saksi ahli, pengacara rekanan, atau stakeholder.
                                </DialogDescription>
                            </div>
                        </div>
                    </DialogHeader>
                        ...
                        <DialogHeader className="border-b border-slate-100 pb-3.5 dark:border-white/[0.06]">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    ...
                                    <DialogDescription className="text-xs text-slate-500 dark:text-zinc-400">
                                        Perbarui foto profil, identitas, dan detail perwakilan.
                                    </DialogDescription>
                                </div>
                            </div>
                        </DialogHeader>
                ...
                <div className="mt-2.5 border-t border-slate-200/70 pt-2.5 dark:border-white/[0.06]">
                    ...
                    <div className="grid grid-cols-5 gap-1.5 rounded-lg bg-slate-100/60 p-1.5 sm:grid-cols-8 dark:bg-zinc-900/60">
                        {AVAILABLE_AVATARS.map((avatar, idx) => {
                            const isSelected = value === avatar.url || (!value && idx === 0);
                            return (
                                <button
                                    key={avatar.id}
                                    type="button"
