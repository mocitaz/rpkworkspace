        ...
        <div class="mx-auto flex w-full max-w-7xl items-center justify-between gap-4">
            <div class="flex items-center gap-3.5">
                ...
                <div class="flex flex-col">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10.5px] font-medium text-slate-500 dark:text-zinc-400 hidden sm:inline">Portal Verifikasi Perkembangan Perkara</span>
                    </div>
                    <span class="text-[10px] text-slate-400 dark:text-zinc-500 truncate max-w-[280px] sm:max-w-none">Roni, Putra &amp; Kusumah Law Firm · Matter Portfolio &amp; Case Governance</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 text-xs font-mono text-slate-500 dark:text-zinc-400">
                <div class="hidden md:flex items-center gap-1.5 text-[11px]">
                    ...
                    <span class="font-medium text-slate-800 dark:text-zinc-200">Manajemen Perkara Resmi</span>
                </div>
                <span class="hidden md:inline text-slate-300 dark:text-zinc-700">·</span>
                @if ($isClosed)
                    <span class="text-[11px] font-sans font-semibold text-slate-600 dark:text-zinc-400">Perkara Selesai (Closed)</span>
                @else
                    <span class="text-[11px] font-sans font-semibold text-emerald-600 dark:text-emerald-400">Perkara Berjalan (Active)</span>
                @endif
            </div>
        </div>
    </header>
    ...
    <main class="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-6 space-y-5 flex-1">
        ...
        <section class="group relative overflow-hidden rounded-[20px] border border-slate-200/80 bg-gradient-to-br from-[#f7f9ff] via-white to-[#eaf3ff] p-6 sm:p-7 shadow-[0_10px_28px_rgba(71,85,105,0.075)] dark:border-white/[0.08] dark:from-[#17191f] dark:via-[#17191f] dark:to-[#18202b]">
            ...
            <div class="relative z-10 space-y-3.5">
                ...
                <div class="flex flex-wrap items-center gap-2 font-mono text-xs text-slate-500 dark:text-zinc-400">
                    <span class="font-bold text-slate-900 dark:text-white uppercase tracking-wider">{{ $matter->matter_number }}</span>
                    ...
                    <span class="text-slate-600 dark:text-zinc-400 font-medium text-xs sm:text-sm">
                        {{ $matter->practiceArea->name ?? 'Litigasi & Korporasi Strategis' }}
                    </span>
                </div>
                ...
                <div class="mt-5 pt-4 border-t border-slate-200/80 dark:border-white/[0.08] grid grid-cols-2 md:grid-cols-4 gap-3.5 md:gap-0 md:divide-x md:divide-slate-200/80 dark:md:divide-white/[0.08]">
                    ...
                    <div class="flex flex-col md:pr-4">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Nomor Perkara</span>
                        <div class="flex items-center gap-1.5 mt-0.5">
                            <span class="font-mono font-bold text-xs sm:text-[13px] text-slate-950 dark:text-white select-all">{{ $matter->matter_number }}</span>
                            <button 
                                type="button" 
                                onclick="copyText('{{ $matter->matter_number }}', this)"
                                title="Salin Nomor Perkara"
                    ...
                    <div class="flex flex-col md:px-4">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Bidang Praktik Hukum</span>
                        <div class="mt-0.5">
                            <span class="text-xs sm:text-[13px] font-bold text-slate-900 dark:text-white truncate">
                    ...
                    <!-- 3. Handling Status -->
                    <div class="flex flex-col md:px-4">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Status Penanganan</span>
                        <div class="flex items-center gap-1.5 mt-0.5">
                            <span class="size-1.5 rounded-full {{ $isClosed ? 'bg-slate-400' : 'bg-emerald-500' }} shrink-0"></span>
                            <span class="text-xs sm:text-[13px] font-bold {{ $isClosed ? 'text-slate-700 dark:text-zinc-300' : 'text-emerald-600 dark:text-emerald-400' }}">
                                {{ $statusLabel }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col md:pl-4">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Tanggal Registrasi</span>
                        <span class="font-mono text-xs sm:text-[12.5px] font-medium text-slate-700 dark:text-zinc-300 mt-0.5">
                            {{ $matter->opened_at ? \Illuminate\Support\Carbon::parse($matter->opened_at)->translatedFormat('d M Y') : ($matter->created_at?->translatedFormat('d M Y') ?? '-') }}
                        </span>
                    </div>
                </div>
            </div>
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white/95 p-6 sm:p-8 shadow-xs backdrop-blur-md dark:border-white/10 dark:bg-[#14161b]/95 space-y-7">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5 sm:gap-6">
                ...
                <div class="rounded-xl border border-slate-200/80 bg-slate-50/50 p-5 dark:border-white/[0.08] dark:bg-zinc-900/40 space-y-3.5">
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-[10.5px] font-mono text-slate-500 dark:text-zinc-400">
                            Tahap: <span class="font-bold text-slate-900 dark:text-white uppercase">{{ $matter->stage ?? 'Pemberkasan' }}</span>
                        </span>
                    </div>
                    ...
                    <div class="pt-2 border-t border-slate-200/70 dark:border-white/[0.06] space-y-2 text-xs">
                        <div class="flex items-center justify-between text-slate-600 dark:text-zinc-400">
                            ...
                            <span class="font-bold text-slate-900 dark:text-white">{{ $matter->client->display_name ?? '-' }}</span>
                        </div>
                        <div class="flex items-center justify-between text-slate-600 dark:text-zinc-400">
                            <span class="text-slate-400 dark:text-zinc-500">Nomor Registrasi:</span>
                            <span class="font-mono font-semibold text-blue-600 dark:text-blue-400">{{ $matter->matter_number }}</span>
                        </div>
                        <div class="flex items-center justify-between text-slate-600 dark:text-zinc-400">
                            <span class="text-slate-400 dark:text-zinc-500">Tanggal Buka Berkas:</span>
                ...
                <!-- Sisi Kanan: Tim Kuasa Hukum Penanggung Jawab -->
                <div class="rounded-xl border border-slate-200/80 bg-slate-50/50 p-5 dark:border-white/[0.08] dark:bg-zinc-900/40 space-y-3.5">
                    ...
                    <div>
                        ...
                        <p class="text-[11px] text-slate-500 dark:text-zinc-400 mt-0.5">
                            Responsible Partner · Kuasa Hukum Utama
                        </p>
                    </div>
                    ...
                    <div class="pt-2 border-t border-slate-200/70 dark:border-white/[0.06] space-y-2 text-xs">
                        @if ($matter->supervisingLawyer)
                            <div class="flex items-center justify-between text-slate-600 dark:text-zinc-400">
                                <span class="text-slate-400 dark:text-zinc-500">Advokat Pelaksana:</span>
                                <span class="font-semibold text-slate-800 dark:text-zinc-200">{{ $matter->supervisingLawyer->name }}</span>
                            </div>
                        @endif
                        <div class="flex items-center justify-between text-slate-600 dark:text-zinc-400">
                            <span class="text-slate-400 dark:text-zinc-500">Otoritas Firma:</span>
                        ...
                        <div class="flex items-center justify-between text-slate-600 dark:text-zinc-400">
                            <span class="text-slate-400 dark:text-zinc-500">Status Penugasan:</span>
                            <span class="font-medium text-slate-800 dark:text-zinc-200">Aktif &amp; Terverifikasi</span>
                        </div>
                    </div>
                </div>
            ...
            @if ($matter->summary)
                <div class="rounded-xl border border-slate-200/80 bg-slate-50/40 p-4 space-y-1.5 text-xs dark:border-white/[0.08] dark:bg-zinc-900/30">
                    <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 block">
                        Ringkasan Posisi &amp; Perkembangan Kasus
                    </span>
            ...
            <div class="pt-4 flex flex-wrap items-center justify-between gap-2 text-xs border-t border-slate-100 dark:border-white/5">
                ...
                <div class="font-mono text-[11px] text-slate-400 dark:text-zinc-500 font-medium">
                    Divisi Manajemen Berkas Perkara
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch">
            ...
            <!-- Kiri: QR Code & Validasi Perkara Instan (7 Cols) -->
            <div class="md:col-span-7 flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-white/95 p-5 sm:p-6 shadow-xs backdrop-blur-md dark:border-white/10 dark:bg-[#14161b]/95">
                ...
                <div class="space-y-1.5 text-xs min-w-0 flex-1">
                    <div class="font-bold text-slate-950 dark:text-white flex items-center gap-1.5">
                        ...
                        <span class="text-xs sm:text-sm">Validasi Perkara Instan</span>
                    </div>
                    <p class="text-slate-500 dark:text-zinc-400 text-[11px] sm:text-xs leading-relaxed">
                        Pindai kode QR untuk memvalidasi keabsahan status penanganan perkara hukum secara publik.
                    </p>
                    <div class="font-mono text-[11px] text-slate-400 dark:text-zinc-500 truncate pt-0.5 select-all">
                        ID: <span class="font-semibold text-slate-800 dark:text-zinc-200">{{ $matter->matter_number }}</span>
                    </div>
                </div>
            </div>

            <!-- Kanan: Aksi & Dokumen Perkara (5 Cols) -->
            <div class="md:col-span-5 flex flex-col justify-between rounded-2xl border border-slate-200/80 bg-white/95 p-5 sm:p-6 shadow-xs backdrop-blur-md dark:border-white/10 dark:bg-[#14161b]/95 space-y-3">
                ...
                <div class="flex flex-col gap-2.5 flex-1 justify-center">
                    ...
                    >
                        <div class="flex items-center gap-2.5">
                            ...
                            <span>Cetak Laporan Perkara</span>
                        </div>
                ...
                <div class="pt-2 flex items-center justify-between text-slate-400 dark:text-zinc-500 font-mono text-[10.5px] border-t border-slate-100 dark:border-white/5">
                    <span class="flex items-center gap-1.5">
                        <span class="size-1.5 rounded-full bg-emerald-500"></span>
                        <span>Sistem Portofolio Sah</span>
                    </span>
    ...
    <footer class="mt-auto border-t border-slate-200/80 bg-white/80 py-4.5 text-center text-xs text-slate-500 backdrop-blur-md transition-colors dark:border-white/[0.06] dark:bg-[#0f1115]/80 dark:text-zinc-400">
        <div class="mx-auto flex w-full max-w-7xl flex-col items-center justify-between gap-3 px-4 sm:flex-row sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-3 font-mono text-[11px] text-slate-400 dark:text-zinc-500">
                <span>Ref: <span class="font-semibold text-slate-600 dark:text-zinc-400">{{ $matter->matter_number }}</span></span>
                <span class="text-slate-300 dark:text-zinc-700">·</span>
