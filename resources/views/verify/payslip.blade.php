            ...
            <div class="flex items-center gap-3.5">
                ...
                <div class="border-l border-slate-200 pl-3">
                    ...
                    <p class="text-[10px] font-semibold text-slate-500">
                        Human Capital &amp; Payroll Management • Bandung
                    </p>
                </div>
            </div>
            ...
            <div>
                @if ($isPaid)
                    <div class="flex items-center gap-2 rounded-xl border border-slate-200/90 bg-slate-50/80 px-3 py-1.5 shadow-2xs">
                        ...
                        <div class="text-left">
                            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Status Pembayaran</div>
                            <div class="text-[11.5px] font-extrabold text-slate-900 leading-tight">Lunas Ditransfer</div>
                        </div>
                    </div>
                @elseif ($isApproved)
                    <div class="flex items-center gap-2 rounded-xl border border-sky-200 bg-sky-50/70 px-3 py-1.5 shadow-2xs">
                        <svg class="size-4 text-sky-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                        </svg>
                        <div class="text-left">
                            <div class="text-[9px] font-bold text-sky-700 uppercase tracking-wider">Status Pembayaran</div>
                            <div class="text-[11.5px] font-extrabold text-sky-900 leading-tight">Disetujui (Approved)</div>
                        </div>
                    </div>
                @else
                    <div class="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-1.5 shadow-2xs">
                        ...
                        <div class="text-left">
                            <div class="text-[9px] font-bold text-slate-500 uppercase tracking-wider">Status Pembayaran</div>
        ...
        <main class="overflow-hidden rounded-3xl border border-slate-200/90 bg-white shadow-xl shadow-slate-200/50 divide-y divide-slate-100">
            ...
            <div class="p-6 sm:p-8 space-y-4 bg-gradient-to-b from-slate-50/80 via-white to-white">
                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
                    <div class="flex items-center gap-2 text-[11px] font-medium text-slate-500">
                        ...
                        <span>•</span>
                        <span>Divisi Penggajian &amp; Sumber Daya Manusia</span>
                    </div>
                ...
                <div>
                    <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">
                        Verifikasi Keabsahan Slip Penghasilan
                    </h1>
                    <p class="mt-1 text-xs sm:text-sm text-slate-600">
                        Dokumen resmi bukti pembayaran kompensasi &amp; honorarium advokat / staf RPK Law Firm.
                    </p>
                </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-slate-200/90 bg-slate-50/60 shadow-2xs grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-200/80">
                    <div class="p-3.5 sm:px-4 sm:py-3 flex flex-col justify-center">
                        <span class="text-[9.5px] font-bold text-slate-400 uppercase tracking-wider">Nomor Slip Gaji</span>
                    ...
                    <div class="p-3.5 sm:px-4 sm:py-3 flex flex-col justify-center">
                        <span class="text-[9.5px] font-bold text-slate-400 uppercase tracking-wider">Periode Penghasilan</span>
                        <span class="mt-0.5 text-xs sm:text-sm font-bold text-slate-900">
                            {{ \Carbon\Carbon::parse($payroll->period . '-01')->translatedFormat('F Y') }}
                        </span>
                    </div>

                    <div class="p-3.5 sm:px-4 sm:py-3 flex flex-col justify-center {{ $isPaid ? 'bg-emerald-50/60' : ($isApproved ? 'bg-sky-50/60' : 'bg-slate-50/60') }}">
                        <span class="text-[9.5px] font-bold uppercase tracking-wider {{ $isPaid ? 'text-emerald-700' : ($isApproved ? 'text-sky-700' : 'text-slate-500') }}">
                            Status Pembayaran
                        </span>
                        <div class="mt-0.5 flex items-center gap-1.5 font-extrabold text-xs sm:text-[13px] {{ $isPaid ? 'text-emerald-950' : ($isApproved ? 'text-sky-950' : 'text-slate-900') }}">
                            @if ($isPaid)
                                <svg class="size-4 text-emerald-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="p-6 sm:p-8 space-y-4">
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="rounded-2xl border border-slate-200/90 bg-slate-50/40 p-4 space-y-3">
                        ...
                        <div class="space-y-1.5 border-t border-slate-200/70 pt-2.5 text-xs text-slate-600">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span class="font-semibold text-slate-800">{{ $payroll->user->position_title ?: 'Advokat / Staf Legal' }}</span>
                            </div>
                            <div class="flex items-center justify-between gap-2">
                                <span class="text-slate-400">Departemen:</span>
                                <span class="font-semibold text-slate-800">{{ $payroll->user->department ?: 'Legal Practice' }}</span>
                            </div>
                            <div class="flex items-center justify-between gap-2">
                                <span class="text-slate-400">Email:</span>
                                <span class="font-mono text-[11px] text-slate-700">{{ $payroll->user->email }}</span>
                            </div>
                            @if ($payroll->user->bank_account_number)
                                <div class="flex items-center justify-between gap-2">
                                    <span class="text-slate-400">Rekening Tujuan:</span>
                                    <span class="font-mono text-[11px] font-semibold text-slate-800">
                                        {{ $payroll->user->bank_name ?? 'Bank' }} {{ $payroll->user->bank_account_number }}
                                    </span>
                                </div>
                            @endif
                        </div>
                    ...
                    <!-- Right: Ringkasan Penghasilan Bersih -->
                    <div class="rounded-2xl border border-slate-200/90 bg-slate-50/40 p-4 space-y-3">
                        <div>
                            <span class="text-[9.5px] font-bold uppercase tracking-wider text-slate-400">
                        ...
                        <div class="space-y-1.5 border-t border-slate-200/70 pt-2.5 text-xs text-slate-600">
                            <div class="flex items-center justify-between gap-2">
                                ...
                                <span class="font-mono font-medium text-slate-800">Rp {{ number_format($payroll->basic_salary, 0, ',', '.') }}</span>
                            </div>
                            @if ($payroll->allowances > 0)
                                <div class="flex items-center justify-between gap-2">
                                    <span class="text-slate-400">Tunjangan &amp; Insentif:</span>
                                    <span class="font-mono font-medium text-emerald-700">+Rp {{ number_format($payroll->allowances, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            @if ($payroll->deductions > 0)
                                <div class="flex items-center justify-between gap-2">
                                    <span class="text-slate-400">Potongan:</span>
                                    <span class="font-mono font-medium text-rose-700">-Rp {{ number_format($payroll->deductions, 0, ',', '.') }}</span>
                                </div>
                            @endif
                            <div class="flex items-center justify-between gap-2 pt-1 border-t border-slate-200/50">
                                <span class="text-slate-400">Rekening Pembayar:</span>
                                <span class="font-semibold text-slate-800 truncate max-w-[200px]">{{ $payroll->paymentAccount->name ?? 'Kas / Bank Operasional Firma' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex items-center gap-3.5">
                        ...
                        <div class="space-y-0.5 text-xs">
                            ...
                            <p class="text-slate-500 text-[11px] leading-tight">
                                Slip gaji ini diterbitkan secara elektronik oleh RPK Law Firm dan sah tanpa tanda tangan basah.
                            </p>
                            <div class="font-mono text-[10px] text-slate-400 truncate max-w-[320px] pt-0.5">
                                Ref: {{ $payroll->payslip_number }}
                            </div>
                        </div>
                    </div>
