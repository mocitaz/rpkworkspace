@php
    ...
    $isApproved = $payroll->status === 'approved';
@endphp
<!doctype html>
...
<body class="min-h-screen text-slate-900 selection:bg-slate-900 selection:text-white dark:text-zinc-100 flex flex-col justify-between">
        ...
        <div class="mx-auto flex w-full max-w-7xl items-center justify-between gap-4">
            <div class="flex items-center gap-3.5">
                ...
                <div class="flex flex-col">
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-[10.5px] font-medium text-slate-500 dark:text-zinc-400 hidden sm:inline">Portal Verifikasi Slip Remunerasi</span>
                    </div>
                    <span class="text-[10px] text-slate-400 dark:text-zinc-500 truncate max-w-[280px] sm:max-w-none">Roni, Putra &amp; Kusumah Law Firm · Human Capital Management</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-3 text-xs font-mono text-slate-500 dark:text-zinc-400">
                <div class="hidden md:flex items-center gap-1.5 text-[11px]">
                    ...
                    <span class="font-medium text-slate-800 dark:text-zinc-200">Sistem Payroll Terdaftar</span>
                </div>
                <span class="hidden md:inline text-slate-300 dark:text-zinc-700">·</span>
                @if ($isPaid)
                    <span class="text-[11px] font-sans font-semibold text-emerald-600 dark:text-emerald-400">Lunas Ditransfer</span>
                @elseif ($isApproved)
                    <span class="text-[11px] font-sans font-semibold text-sky-600 dark:text-sky-400">Disetujui (Approved)</span>
                @else
    ...
    <main class="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8 py-6 space-y-5 flex-1">
            ...
            <div class="relative z-10 max-w-5xl">
                ...
                <div class="flex flex-wrap items-center gap-2 text-xs mb-1">
                    ...
                    <span class="text-slate-600 dark:text-zinc-400 font-medium text-xs sm:text-sm">
                        Divisi Penggajian &amp; Sumber Daya Manusia
                    </span>
                </div>
                ...
                <div class="mt-5 pt-4 border-t border-slate-200/80 dark:border-white/[0.08] grid grid-cols-2 md:grid-cols-4 gap-3.5 md:gap-0 md:divide-x md:divide-slate-200/80 dark:md:divide-white/[0.08]">
                    ...
                    <div class="flex flex-col md:pr-4">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Nomor Slip Gaji</span>
                        <div class="flex items-center gap-1.5 mt-0.5">
                            <span class="font-mono font-bold text-xs sm:text-[13px] text-slate-950 dark:text-white select-all">{{ $payroll->payslip_number }}</span>
                            <button 
                                type="button" 
                                onclick="copyText('{{ $payroll->payslip_number }}', this)"
                                title="Salin Nomor Slip"
                    ...
                    <div class="flex flex-col md:px-4">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Periode Penghasilan</span>
                        <div class="flex items-center gap-1.5 mt-0.5">
                    ...
                    <div class="flex flex-col md:px-4">
                        ...
                        <div class="mt-0.5">
                            ...
                            @elseif ($isApproved)
                                <span class="text-xs sm:text-[13px] font-bold text-sky-600 dark:text-sky-400">Disetujui (Approved)</span>
                            @else
        ...
        <div class="rounded-2xl border border-slate-200/80 bg-white/95 p-6 sm:p-7 shadow-xs backdrop-blur-md dark:border-white/10 dark:bg-[#14161b]/95 space-y-6">
                ...
                <div class="flex items-center gap-3.5">
                    ...
                    <div>
                        <h2 class="text-sm sm:text-base font-black tracking-tight text-slate-950 dark:text-white leading-tight">
                            Slip Penghasilan Sah &amp; Terotentikasi
                        </h2>
                        <p class="text-[11.5px] text-slate-500 dark:text-zinc-400 mt-0.5">
                            Dokumen Resmi Bukti Pembayaran Kompensasi Advokat &amp; Staf RPK Law Firm
                        </p>
                    </div>
                </div>

                <p class="text-xs sm:text-[13px] text-slate-600 dark:text-zinc-300 leading-relaxed font-normal">
                    Slip gaji elektronik ini diterbitkan secara sah oleh RPK Law Firm dan diakui secara hukum tanpa memerlukan tanda tangan basah.
                </p>
            </div>
            ...
            <!-- 2. Employee Info & Take Home Pay -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="space-y-1 text-xs">
                    <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Penerima Penghasilan</span>
                    <div class="font-bold text-slate-950 dark:text-white text-sm sm:text-base">
                        {{ $payroll->user->name }}
                    </div>
                    <div class="text-[11.5px] text-slate-600 dark:text-zinc-400 font-medium">
                        {{ $payroll->user->position_title ?: 'Advokat / Staf Legal' }} · {{ $payroll->user->department ?: 'Legal Practice' }}
                    </div>
                    @if ($payroll->user->email)
                        <div class="font-mono text-[10.5px] text-slate-400 dark:text-zinc-500 pt-0.5">{{ $payroll->user->email }}</div>
                    @endif
                    @if ($payroll->user->bank_account_number)
                        <div class="font-mono text-[11px] text-slate-600 dark:text-zinc-300 pt-0.5">
                            Rekening Tujuan: {{ $payroll->user->bank_name ?? 'Bank' }} {{ $payroll->user->bank_account_number }}
                        </div>
                    @endif
                </div>

                <div class="space-y-1 text-xs">
                    <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500">Penghasilan Bersih (Take Home Pay)</span>
                    ...
                    <div class="text-[11px] text-slate-500 dark:text-zinc-400 pt-0.5">
                        Rekening Pembayar: <span class="font-semibold text-slate-700 dark:text-zinc-300">{{ $payroll->paymentAccount->name ?? 'Kas / Bank Operasional Firma' }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="space-y-2.5">
                <span class="text-[11px] font-bold uppercase tracking-wider text-slate-500 dark:text-zinc-400">
                    Rincian Komponen Penghasilan &amp; Potongan
                </span>

                <div class="rounded-xl border border-slate-200/80 dark:border-white/10 overflow-hidden text-xs">
                    <div class="divide-y divide-slate-100 dark:divide-white/[0.06] bg-white dark:bg-transparent">
                        <div class="p-3.5 flex justify-between items-center">
                            <span class="font-medium text-slate-900 dark:text-zinc-100">Gaji Pokok / Imbalan Dasar</span>
                            <span class="font-mono font-semibold text-slate-900 dark:text-zinc-100">Rp {{ number_format($payroll->basic_salary, 0, ',', '.') }}</span>
                        </div>
                        @if ($payroll->allowances > 0)
                            <div class="p-3.5 flex justify-between items-center text-emerald-600 dark:text-emerald-400">
                                <span>Tunjangan &amp; Insentif Khusus</span>
                                <span class="font-mono font-semibold">+Rp {{ number_format($payroll->allowances, 0, ',', '.') }}</span>
                            </div>
                        @endif
                        @if ($payroll->deductions > 0)
                            <div class="p-3.5 flex justify-between items-center text-rose-600 dark:text-rose-400">
                                <span>Potongan (Pajak / Asuransi / Lainnya)</span>
                                <span class="font-mono font-semibold">-Rp {{ number_format($payroll->deductions, 0, ',', '.') }}</span>
                            </div>
                        @endif
                        <div class="p-4 bg-slate-50/70 dark:bg-zinc-900/40 border-t border-slate-200/80 dark:border-white/10 flex justify-between items-baseline font-black text-sm text-slate-900 dark:text-white">
                            <span class="uppercase tracking-wider text-xs">TOTAL DITERIMA (NET):</span>
                            <span class="font-mono text-base font-black text-slate-950 dark:text-white">Rp {{ number_format($payroll->net_salary, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch">
            ...
            <!-- Kiri: QR Code & Validasi Slip Gaji Instan (7 Cols) -->
            <div class="md:col-span-7 flex items-center gap-4 rounded-2xl border border-slate-200/80 bg-white/95 p-5 sm:p-6 shadow-xs backdrop-blur-md dark:border-white/10 dark:bg-[#14161b]/95">
                <div class="shrink-0 p-2.5 rounded-xl bg-white dark:bg-zinc-950 border border-slate-200/90 dark:border-white/10 shadow-2xs">
                    <img 
                        src="{{ route('verify.payslip.qr', $payroll->payslip_number) }}" 
                        alt="QR Code Verifikasi Slip Gaji" 
                ...
                <div class="space-y-1.5 text-xs min-w-0 flex-1">
                    <div class="font-bold text-slate-950 dark:text-white flex items-center gap-1.5">
                        ...
                        <span class="text-xs sm:text-sm">Validasi Slip Gaji Instan</span>
                    </div>
                    <p class="text-slate-500 dark:text-zinc-400 text-[11px] sm:text-xs leading-relaxed">
                        Pindai kode QR untuk memvalidasi keabsahan slip remunerasi secara elektronik dan publik.
                    </p>
                    <div class="font-mono text-[11px] text-slate-400 dark:text-zinc-500 truncate pt-0.5 select-all">
                        ID: <span class="font-semibold text-slate-800 dark:text-zinc-200">{{ $payroll->payslip_number }}</span>
                    </div>
                </div>
            </div>

            <!-- Kanan: Aksi & Dokumen Slip Gaji (5 Cols) -->
            <div class="md:col-span-5 flex flex-col justify-between rounded-2xl border border-slate-200/80 bg-white/95 p-5 sm:p-6 shadow-xs backdrop-blur-md dark:border-white/10 dark:bg-[#14161b]/95 space-y-3">
                ...
                <div class="flex flex-col gap-2.5 flex-1 justify-center">
                    ...
                    >
                        <div class="flex items-center gap-2.5">
                            ...
                            <span>Cetak Slip Penghasilan</span>
                        </div>
                ...
                <div class="pt-2 flex items-center justify-between text-slate-400 dark:text-zinc-500 font-mono text-[10.5px] border-t border-slate-100 dark:border-white/5">
                    <span class="flex items-center gap-1.5">
                        <span class="size-1.5 rounded-full bg-emerald-500"></span>
                        <span>Sistem Penggajian Sah</span>
                    </span>
    ...
    <footer class="mt-auto border-t border-slate-200/80 bg-white/80 py-4.5 text-center text-xs text-slate-500 backdrop-blur-md transition-colors dark:border-white/[0.06] dark:bg-[#0f1115]/80 dark:text-zinc-400">
        <div class="mx-auto flex w-full max-w-7xl flex-col items-center justify-between gap-3 px-4 sm:flex-row sm:px-6 lg:px-8">
            ...
            <div class="flex items-center gap-3 font-mono text-[11px] text-slate-400 dark:text-zinc-500">
                <span>Ref: <span class="font-semibold text-slate-600 dark:text-zinc-400">{{ $payroll->payslip_number }}</span></span>
                <span class="text-slate-300 dark:text-zinc-700">·</span>
