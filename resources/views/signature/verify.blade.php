            ...
            <div class="flex items-center gap-3.5">
                ...
                <div class="border-l border-slate-200 pl-3">
                    ...
                    <p class="text-[10px] font-semibold text-slate-500">
                        Digital Signature &amp; Legal Technology • Bandung
                    </p>
                </div>
            </div>
                ...
                @if ($isCompleted)
                    <div class="flex items-center gap-2 rounded-xl border border-slate-200/90 bg-slate-50/80 px-3 py-1.5 shadow-2xs">
                        ...
                        <div class="text-left">
                            ...
                            <div class="text-[11.5px] font-extrabold text-slate-900 leading-tight">Tanda Tangan Sah Lengkap</div>
                        </div>
                    </div>
                @elseif ($isPending)
                    <div class="flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50/70 px-3 py-1.5 shadow-2xs">
                        ...
                        <div class="text-left">
                            ...
                            <div class="text-[11.5px] font-extrabold text-amber-900 leading-tight">Menunggu Penandatanganan</div>
                        </div>
                    </div>
                @else
                    <div class="flex items-center gap-2 rounded-xl border border-slate-200 bg-slate-50/70 px-3 py-1.5 shadow-2xs">
                        ...
                        <div class="text-left">
                            <div class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Status Validasi</div>
                            <div class="text-[11.5px] font-extrabold text-slate-800 leading-tight">{{ str($signatureRequest->status)->replace('_', ' ')->title() }}</div>
                        </div>
                    </div>
                @endif
            </div>
            ...
            <div class="p-6 sm:p-8 space-y-4 bg-gradient-to-b from-slate-50/80 via-white to-white">
                <div class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 pb-3">
                    <div class="flex items-center gap-2 text-[11px] font-medium text-slate-500">
                        ...
                        <span>•</span>
                        <span>Divisi Sertifikasi Tanda Tangan Elektronik</span>
                    </div>
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs sm:text-sm text-slate-600">
                        Sertifikat verifikasi keabsahan penandatanganan elektronik dokumen hukum (UU ITE Compliant).
                    </p>
                </div>
                ...
                <div class="overflow-hidden rounded-2xl border border-slate-200/90 bg-slate-50/60 shadow-2xs grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-slate-200/80">
                    <div class="p-3.5 sm:px-4 sm:py-3 flex flex-col justify-center">
                        ...
                        <span class="mt-0.5 font-mono text-xs sm:text-sm font-black text-slate-900 truncate select-all">
                            {{ $signatureRequest->verification_code }}
                        </span>
                    </div>

                    <div class="p-3.5 sm:px-4 sm:py-3 flex flex-col justify-center">
                        <span class="text-[9.5px] font-bold text-slate-400 uppercase tracking-wider">Perkara / Versi</span>
                        <span class="mt-0.5 text-xs sm:text-sm font-bold text-slate-900 truncate">
                            @if ($signatureRequest->document->matter)
                                <span class="font-mono text-blue-600">{{ $signatureRequest->document->matter->matter_number }}</span> (v{{ $signatureRequest->documentVersion->version_number ?? 1 }}.0)
                            @else
                    ...
                    <div class="p-3.5 sm:px-4 sm:py-3 flex flex-col justify-center {{ $isCompleted ? 'bg-emerald-50/60' : 'bg-amber-50/60' }}">
            ...
            <div class="p-6 sm:p-8 space-y-4">
                                ...
                                <div class="space-y-1.5 sm:text-right text-xs">
                                    ...
                                    @if ($signer->page_number)
                                        <div>
                                            ...
                                            <span class="font-mono text-slate-700">Halaman {{ $signer->page_number }}</span>
                                        </div>
                                    @endif
                                </div>
                            </div>
                ...
                @if ($isCompleted)
                    <div class="rounded-2xl border border-slate-200/90 bg-slate-50/70 p-4 space-y-2.5">
                        <div class="text-xs font-extrabold text-slate-900">Unduh Berkas Resmi Yang Telah Dibubuhi</div>
                        <div class="flex flex-wrap gap-2">
                            <a
                                href="{{ route('signature.verify.download-signed', $signatureRequest->verification_code) }}"
                                class="inline-flex items-center gap-1.5 rounded-xl bg-slate-900 px-3.5 py-2 text-xs font-semibold text-white shadow-2xs hover:bg-slate-800 active:scale-95 transition-all"
                            >
                            ...
                            <a
                                ...
                                class="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-800 shadow-2xs hover:bg-slate-50 active:scale-95 transition-all"
                            >
                ...
                <!-- Checksum & Legal Notice -->
                <div class="rounded-xl border border-slate-200 bg-slate-50/60 p-3.5 space-y-1.5 text-xs text-slate-600">
                    <div class="flex items-center justify-between">
                        <span class="text-[9.5px] font-bold uppercase tracking-wider text-slate-400">DIGITAL CHECKSUM (SHA-256)</span>
                        <button 
                            type="button" 
                    ...
                    <code class="block font-mono text-[11px] text-slate-800 font-bold break-all select-all">
